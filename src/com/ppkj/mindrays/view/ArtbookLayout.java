...
public class ArtbookLayout extends FreeFlowLayoutBase implements FreeFlowLayout {
	...
	public void prepareLayout() {
		...
		for (int i = 0; i < (s.getDataCount() > 5 ? 5 : s.getDataCount()); i++) {
			rowIndex = i / 5;
		...
		if (s.getDataCount() > 5) {
			for (int i = 5; i < s.getDataCount(); i++) {
				rowIndex = i / 4 + 1;
				...
				switch (i % 4) {
				case (1):
					r.left = 0 * regularItemSide;
					r.right = 1 * regularItemSide;
					r.top = (rowIndex + 0) * regularItemSide;
					r.bottom = r.top + regularItemSide;
					break;
				case (2):
					r.left = 1 * regularItemSide;
					r.right = 2 * regularItemSide;
					r.top = (rowIndex + 0) * regularItemSide;
					r.bottom = r.top + regularItemSide;
					break;
				case (3):
					r.left = 2 * regularItemSide;
					r.right = 3 * regularItemSide;
					r.top = (rowIndex + 0) * regularItemSide;
					r.bottom = r.top + regularItemSide;
					break;
				case (0):
					r.left = 3 * regularItemSide;
					r.right = 4 * regularItemSide;
					r.top = (rowIndex - 1) * regularItemSide;
					r.bottom = r.top + regularItemSide;
					break;
	...
	public HashMap<Object, FreeFlowItem> getItemProxies(int viewPortLeft,
		...
		// Log.d(TAG,
		// "Viewport: "+viewPortLeft+", "+viewPortTop+", "+viewport.width()+","+viewport.height());
	...
	public int getContentHeight() {
		...
		return ((s.getDataCount() - 5) / 4 + 1) * regularItemSide
				+ largeItemSide;
